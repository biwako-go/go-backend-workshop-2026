...
- バトルで自分のHPが0になると**GAME OVER**。HPは全回復してステージ選択に戻るので、何度でも挑み直せる
...
## Lv3：強敵に一撃でやられる！HPを盛りたい

**症状：** ステージ3「城」のダークナイト（攻撃120）に挑むと、素のHP100では**一撃でGAME OVER**になってしまう。HPを増やそうにも、ゲーム画面の「HP編集」ボタンを押すと「APIが見つかりません」とエラーになる。`PUT /api/hero/hp` のルートが登録されていないためだ。
...
**クリア判定：** 「HP編集」ボタンで「HPを◯◯に設定しました！」と表示されればOK。HPを700以上に盛ってダークナイトに挑もう（最奥のボスドラゴン戦は999推奨）。
